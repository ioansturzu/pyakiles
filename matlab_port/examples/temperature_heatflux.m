...
function temperature_heatflux(test_mode)
  ...
  userdata.akiles2d.simdir = fullfile(pwd, 'matlab_port', 'examples', 'sims_temperature_heatflux');
  ...
  title('Electron temperatures');
  saveas(gcf, fullfile(userdata.akiles2d.simdir, 'temperatures.png'));
  ...
  xlim([1, max(sol.h(idx_finite))]);
  title('Axial heat flux');
  saveas(gcf, fullfile(userdata.akiles2d.simdir, 'heatflux.png'));
  ...
  fid = fopen(fullfile(userdata.akiles2d.simdir, 'temperature_heatflux_results.json'), 'w');
