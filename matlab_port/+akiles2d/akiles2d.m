...
VERSION = '20200901';
% import akiles2d.logger.* removed
...
%% Startup messages
akiles2d.logger.title(['AKILES2D version ',VERSION],10,data.logger);
akiles2d.logger.log(['Simulation directory: ',data.akiles2d.simdir],'INF',5,data.logger);
akiles2d.logger.log(['Structure data saved to ',data.akiles2d.datafile,' successfully.'],'INF',5,data.logger);
...
for iiter = 0:data.akiles2d.maxiter
    ...
    akiles2d.logger.log(['Iteration ',num2str(iiter),' saved to disk.'],'INF',5,data.logger);
    akiles2d.logger.log(['Error norm: ',num2str(normerror)],'INF',4,data.logger); 
    akiles2d.logger.log('Detailed error vector:','INF',1,data.logger);
    ...
    if norm(solution.errorfcn) < data.akiles2d.tolerance
        akiles2d.logger.log(['Convergence reached successfully at iteration ',num2str(iiter),'.'],'INF',5,data.logger);
         ...
         save(solutionfile,'-struct','solution');
         akiles2d.logger.log(['Final iteration saved to ',solutionfile],'INF',5,data.logger);
        break;
    end
...
if iiter == data.akiles2d.maxiter
    akiles2d.logger.log('Maximum number of iterations was reached!','WRN',8,data.logger);
end

%% Run postprocessor functions and save
postfunctions = data.postprocessor.postfunctions;
for ifn = 1:length(postfunctions)
    akiles2d.logger.log(['Running postprocessor function: ',postfunctions{ifn}],'INF',5,data.logger);        
    solution = akiles2d.postprocessor.(postfunctions{ifn})(data,solution);
end
solutionfile = fullfile(data.akiles2d.simdir,'post.mat');
save(solutionfile,'-struct','solution');    
akiles2d.logger.log(['Solution saved to ',solutionfile],'INF',5,data.logger);
